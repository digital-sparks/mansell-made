(() => {
  ...
  window.Webflow.push(() => {
    ...
    matchMedia2.add("(min-width: 768px)", () => {
      const heroImagesParallax = gsapWithCSS.timeline({
        scrollTrigger: {
    ...
    let workCarouselInit = false, workCarousel;
    const workCarouselClasNames = ["work-swiper_wrapper", "work-swiper_list", "work-swiper_slide"];
    function initWorkSwiper() {
      if (window.innerWidth > 767) {
        if (!workCarouselInit) {
          document.querySelectorAll(`.${workCarouselClasNames[2]}`).forEach((slide2) => {
            document.querySelector(`.${workCarouselClasNames[1]}`).append(slide2.cloneNode(true));
          });
    ...
    gsapWithCSS.from(".work-swiper_component .work-item_component img", {
      opacity: 0,
      duration: 0.8,
      scrollTrigger: {
    ...
    document.querySelectorAll(".work-item_component").forEach((item) => {
      ...
      gsapWithCSS.set(foregroundImage, {
      ...
      images.forEach((image) => {
        gsapWithCSS.set(image, { autoAlpha: 0 });
      });
      gsapWithCSS.set(images[currentIndex], { autoAlpha: 1 });
      item.addEventListener("mouseover", () => {
        intervalFunction = setInterval(() => {
          gsapWithCSS.to(images[currentIndex], { autoAlpha: 0, duration: 0 });
          currentIndex = (currentIndex + 1) % images.length;
          gsapWithCSS.to(images[currentIndex], { autoAlpha: 1, duration: 0 });
        }, 240);
        gsapWithCSS.timeline().to(foregroundImageWrap, {
          scale: 1.015,
          ease: "power2.out",
          duration: 0.5
        }).to(
    ...
    document.querySelectorAll(".discover_list .discover_item").forEach((item, i) => {
      item.addEventListener("mouseenter", () => {
        gsapWithCSS.to(".discover_image-list", {
